...
class Comments(models.Model):
    ...
    id = models.AutoField(primary_key=True)
    commenter = models.ForeignKey(User, on_delete=models.CASCADE, null=True, blank=True, related_name="comments")
    listed_item = models.ForeignKey(Listing, on_delete=models.CASCADE, null=True, blank=True, related_name="comments")
    comment = models.CharField(max_length=128)
    time_stamp = models.DateTimeField(auto_now_add=True, null=True, blank=True)

    def __str__(self):
        return f" {self.commenter}: {self.listed_item}"
